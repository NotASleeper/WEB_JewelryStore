...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('addNewWarrantyForm').addEventListener('click', function () {
...
function displayWarrantyList(list) {
    ...
    list.forEach(async warranty => {
        ...
        row.getElementById('customer').textContent = await getCustomerByID(warranty.id_customer).name;
        row.getElementById('date').textContent = formatDate(warranty.date_created);
        row.getElementById('product').textContent = await getProductByID(warranty.id_product).name;
        row.getElementById('content').textContent = await getActivityByID(warranty.id_activity).name_activity;
...
async function getActivityByID(id) {
    try {
        const response = await fetch(`http://localhost:5501/api/v1/service-activities?id=${id}`);
        if (!response.ok) {

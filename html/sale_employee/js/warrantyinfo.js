...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (mode === "add") {
    ...
    addMode();
    document
      .getElementById("searchCustomerInfo")
    ...
    document
      .getElementById("searchProductInfo")
      .addEventListener("keydown", async function (e) {
        if (e.key !== "Enter") return;
    ...
    document
      ...
      .addEventListener("change", async function () {
        ...
        const surcharge =
          ...
          document.getElementById("surcharge").value === "Free"
            ? 0
            : parseInt(document.getElementById("surcharge").value);
        document.getElementById("total").textContent =
          activity.price + surcharge;
      });

    document
      .getElementById("surcharge")
      .addEventListener("change", function () {
        let price = activity ? activity.price : 0;
        const surcharge =
          document.getElementById("surcharge").value === "" ||
          document.getElementById("surcharge").value === "Free"
            ? 0
            : parseInt(document.getElementById("surcharge").value);
        document.getElementById("total").textContent = price + surcharge;
      });
  ...
  fetch(`http://localhost:5501/api/v1/warranty-maintainances/${id}`)
    ...
    .then(async (data) => {
      ...
      if (data.image) {
        ...
        document.getElementById("fr-image").style.display = "none";
      }
    })
    .catch((error) => {
  ...
  order.forEach(async (o) => {
    ...
    detail.forEach(async (d) => {
      if (d.id_product === productID) {
        ...
        document.getElementById("warranty_period").textContent =
    ...
    if (t) {
      document.getElementById("warranty_period").textContent = "None";
    }
  });
}
...
async function createWarranty(customerID, productID, activityID, type) {
  ...
  try {
    const response = await fetch(
      "http://localhost:5501/api/v1/warranty-maintainances",
      {
        method: "POST",
        headers: {
...
async function getDetailByID(id) {
  try {
    const response = await fetch(
      `http://localhost:5501/api/v1/order-details/form/${id}`
    );
    if (!response.ok) {
...
async function getOrderByCustomerID(id) {
  try {
    const response = await fetch(
      `http://localhost:5501/api/v1/order-forms/id-customer/${id}`
    );
    if (!response.ok) {
...
async function getActivity() {
  try {
    const response = await fetch(
      `http://localhost:5501/api/v1/service-activities`
    );
    if (!response.ok) {
...
async function getCategoryByID() {
  try {
    const response = await fetch(
      `http://localhost:5501/api/v1/product-categories/${id}`
    );
    if (!response.ok) {
      throw new Error("Failed to fetch categories");
    }
